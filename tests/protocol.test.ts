...
test("SHA-256 verification rejects changed file bytes", async () => {
  ...
  const recovered = await unpackFile(packed.container);
  recovered.bytes[0] = recovered.bytes[0]! ^ 0xff;
...
test("frames that are not ours, or not self-consistent, are rejected", () => {
  ...
  const oversizedContainer = good.slice();
  new DataView(oversizedContainer.buffer).setUint32(12, MAX_CONTAINER_BYTES + 1, true);
  assert.equal(parseFrame(oversizedContainer), null, "hostile containers must not allocate past 64 MiB");
});
