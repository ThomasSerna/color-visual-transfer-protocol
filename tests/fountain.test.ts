...
test("a receiver joining well into the infinite stream reconstructs without early frames", () => {
  const byteLength = 180_000;
  const blockLen = 1_298;
  const sessionId = 0x4d1d;
  const firstObservedSequence = 25_000;
  const payload = testPayload(byteLength);
  const encoder = new LTEncoder(payload, blockLen, sessionId);
  const decoder = new LTDecoder(encoder.k, blockLen, sessionId, byteLength);

  // The receiver deliberately never sees sequence 0 or any earlier frame. A
  // Decimen stream has no START frame: every sequence number independently
  // identifies its fountain equation, so joining an already-running sender is
  // equivalent to beginning at any other point in the stream.
  const ceiling = firstObservedSequence + encoder.k * 20 + 2_000;
  let sequence = firstObservedSequence;
  while (!decoder.isComplete && sequence < ceiling) {
    decoder.addFrame(sequence, encoder.encode(sequence));
    sequence++;
  }

  assert.ok(decoder.isComplete, `mid-stream receiver stalled after ${sequence - firstObservedSequence} frames`);
  assert.ok(decoder.framesNew >= encoder.k);
  assert.deepEqual(decoder.assemble(), payload);
});
