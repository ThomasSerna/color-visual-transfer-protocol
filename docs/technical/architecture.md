# Architecture

Three pages, one shared core, a handful of single-purpose build plugins. No framework, no state library — each page is one TypeScript module wiring DOM to the shared code.

## Pages

| dir | page | entry |
|---|---|---|
| `/` | asymmetric signal field, Transmit/Capture roles, share dialog | `home/main.ts` |
| `send/` | workbench controls → fountain worker → optical stage | `send/main.ts`, carrier workers |
| `receive/` | dominant camera preview → selected carrier worker → diagnostic rail | `receive/main.ts`, carrier workers |

## Shared modules (`shared/`)

- `fountain.ts` — LT encoder/decoder, deterministic soliton distribution (see [Protocol](protocol.md)).
- `send/fountain-*` — session-scoped worker that owns `LTEncoder.encode` and
  `packFrame`; the UI thread only schedules a three-frame lookahead and paints
  completed rasters. The same worker is embedded in the QR-only standalone sender.
- `protocol.ts` — frame header pack/parse, file container, SHA-256 verification, stream identity.
- `frame-capacity.ts` — QR capacity math: payload size → block length / count limits.
- `qr-raster.ts` — QR module matrix → RGBA raster.
- `carrier.ts` — the `QR_LEGACY` / `COLOR_4` encoder and decoder boundary.
- `color4/` — profile registry, outer PDU, CRC/FEC/interleaving/whitening,
  frozen physical layout and canonical raster codec.
- `experiments.ts` — counters, latency summaries and JSON export;
  only completed summaries and user preferences may enter IndexedDB.
- `display.ts` — QR display-size fitting against the viewport.
- `platform.ts` — `isIOS`/`isAndroid` sniffs and camera capability probing (torch, continuous focus, max fps). Policy: probe wherever probeable; sniff only for unprobeable behavior.
- `worker-pool.ts` — decode worker pool; busy workers drop frames, the fountain absorbs it.
- `no-signal.ts` — pure timing policy for the "Nothing happening?" hint (short first delay, longer after dismissal).
- `progress.ts` — frames-collected progress estimation and fountain-overhead model.
- `send-settings.ts` — canonical tx settings lists; the sender's dropdowns and the no-signal advice both render from it.
- `snippet.ts` — text-snippet container type.
- `dialog.ts` — geometric backdrop-click close for `<dialog>`.
- `share-dialog.ts` — the share dialog both home and sender carry (QR + copy + OS sheet).
- `status-line.ts`, `wake-lock.ts`, `format.ts`, `style.css`.

## Build plugins (`build/`)

One file each, exact-match string surgery that **throws when it misses** — markup drift breaks the build instead of shipping broken output.

- `html-tokens.ts` — `%TOKEN%` substitution (site URL, settings options, version, build id).
- `root-pwa-head.ts` — owns the manifest link and SW registration on every page; validates the URLs resolve to the site root under any subpath.
- `rewrite-standalone-links.ts` — strips/rewrites hosted-site references for the single-file builds.
- `inline-zxing-wasm.ts`, `use-inline-variants.ts` — inline the decoder wasm/worker for standalone.
- `standalone-csp.ts`, `emit-as.ts` — standalone CSP and output naming.
- `make-icons.ts` — renders the PWA icons and social card from the CVTP SVG with pinned `@resvg/resvg-js` (`npm run icons`).

Typechecking: `tsconfig.json` covers the pages and `shared/`; `tsconfig.node.json` covers `build/` and `vite.config.ts`; `tsconfig.test.json` covers unit tests, Playwright and its generated camera fixture. All run in `npm run typecheck` and therefore in every build.

## Carrier isolation

The sender and receiver explicitly select one carrier. QR loads `qrcode` or
the ZXing worker; COLOR_4 loads its renderer or the single OpenCV worker. They
are never run concurrently. A profile or palette change cancels the in-memory
session and starts with a new fountain session id.

The hosted PWA contains both carriers and precaches their lazy chunks and WASM.
Standalone builds are deliberately QR-only and compile out every COLOR_4 and
OpenCV path. Active transfer data and LT equations stay in RAM: reload, close
or carrier change discards them.

## Product shell

The public interface is the CVTP **Signal Workshop**: system sans for product
copy, monospace only for hashes and telemetry, and K/C/M/Y signal accents over
graphite surfaces. The optical payload surfaces are deliberately outside that
visual treatment: `#qr`, `#stage` and the receiver video receive no filters,
decorative transforms or interpolation. Idle motion stops while
`transfer-active` is set and under `prefers-reduced-motion`.
