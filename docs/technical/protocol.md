...
COLOR_4 is an alternative outer carrier, not a second file protocol. It wraps
one exact 20-byte-header Decimen frame with a versioned header and CRC32C,
applies Reed–Solomon, interleaving and whitening, then renders dibits as four
colours. Successful decoding removes that wrapper and hands the original bytes
to this same parser and fountain decoder. See the normative
[COLOR_4/1 specification](color4-protocol.md).
