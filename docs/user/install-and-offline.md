...
Three shapes, all built from the same source. Built artifacts for all three are attached to every [release](https://github.com/ThomasSerna/color-visual-transfer-protocol/releases).

| | what it is | needs a server? | offline |
|---|---|---|---|
| **Hosted site** | three pages plus a service worker — default [GitHub Pages deployment](https://thomasserna.github.io/color-visual-transfer-protocol/) | yes, any static host | after the first visit |
| **`cvtp-sender.html`** | one file, ~100 KB | no | always |
| **`cvtp-receiver.html`** | one file, ~1.3 MB | see the caveat | always |

## Hosted site: install and offline

The site precaches everything, including ZXing WASM and the lazy OpenCV bundle — load it once and it works with the network off. Any page does it; landing straight on `/receive/` from a shared link caches the whole app.
...
Both standalone files are deliberately **QR_LEGACY-only**. COLOR_4 and OpenCV
exist only in the hosted/PWA build so the self-contained receiver stays small
and preserves the QR interoperability path with Decimen v0.3.0.
